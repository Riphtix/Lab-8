...
public class Deck {
	...
	public Card[] select(int k) {
		Card[] results = new Card[k];
		
		for(int i = 0; i < k; i++){
			int pos = rand.nextInt(cards.length - (i + 1));
			Card temp = cards[cards.length - (i + 1)];
			cards[cards.length - (i + 1)] = cards[pos];
			cards[pos] = temp;
		}
		
		for(int i = 0; i < k; i++){
			results[i] = cards[cards.length - (i + 1)];
		}
